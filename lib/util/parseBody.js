        ...
        switch (request.body.mode) {
            case 'raw':
                if (!_.isEmpty(request.body[request.body.mode])) {
                    body += `$body->append(${JSON.stringify(request.body[request.body.mode])});\n`;
                }
                return body;
            ...
            case 'urlencoded':
                ...
                if (!_.isEmpty(list)) {
                    temp = _.map(list, function (value) {
                        return `${indentation}'${value.key.replace(/'/g, '\\\'')}' => '${value.value.replace(/'/g, '\\\'')}'`;
                    });
            ...
            case 'formdata':
                ...
                if (!_.isEmpty(list)) {
                    temp = _.map(_.filter(list,{'type': 'text'}), function (value) {
                            return (`${indentation}'${value.key}' => '${value.value.replace(/\\/g, '\\\\')}'`);
                    });
